...
- ~~`run_strategic_optuna_sweep`~~ — **NOT AVAILABLE in this ablation campaign. Use `run_training_trial` only.**
...
2. **INNOVATE**: If parameter optimization fails to break the plateau (approx. 20+ steps), you must design your own custom architecture. Use propose_intelligent_architecture. This activates the Innovation Team to generate a novel architectures.
...
⚠️ **UPDATED THRESHOLDS (Aggressive Escalation)**:
- `initializing`: < 10 trials
- `plateauing_try_harder`: 10-20 steps stuck
- `plateauing_innovate`: 20+ steps stuck
...
3. **Decide Based on Status**:
   ...
   - `plateauing_try_harder` (10-20 steps) → **ESCALATE** to varied `run_training_trial` configs or architectural mutation
   - `plateauing_innovate` (20+ steps) → **INNOVATE with innovation team** (Step 4c)
...
4. **Act**:
   ...
   - **4b (Escalate):** run multiple varied `run_training_trial` with diverse hyperparameter configs or `propose_architectural_mutation()`
   - **4c (Innovate):** `generate_creative_hypotheses(query: str)` and `propose_intelligent_architecture()`    
5. **Flag & Switch** (only if stopping):
   ```json
...
- **IF A NEW MODEL IS FOUND:** If the history shows `[PostSupernetAgent Report]` or that a new model file has been constructed, **YOU MUST SWITCH TO EXPLOITATION.**
  - **Goal:** "A new Genotype has been automatically constructed and verified. Goal: Run targeted `run_training_trial` experiments to optimize this new architecture."
...
- **Minimum 10-15 trials** completed (no premature judgments!)
- Thresholds: `initializing` < 10 trials, `plateauing_try_harder` 10-20 steps, `plateauing_innovate` 20+ steps

**Step 2: Mandatory Advanced Strategies**
   - Multiple varied `run_training_trial` calls with diverse hyperparameter ranges
...
**Step 3:  MANDATORY - Supernet Rescue Attempt **
   - **IF** an architecture is plateauing (20+ steps stuck), you **MUST** run `run_supernet_search()`
...
**Step 4: Abandon Criteria (ALL must be true)**
   ...
   - ✓ Multiple varied `run_training_trial` batches failed to improve
...
{{"tool_name": "get_optimization_status", "args": {{}}}}
// Result: "1D_CNN" is "plateauing_innovate" (21+ steps stuck)
...
---
**AUTONOMY GUIDELINES:**
- **Use `run_training_trial`** for all exploration — Optuna is not available in this campaign
...
→ Do NOT propose another set of training trials on the same config. Switch architecture instead.
...
→ H1 MUST propose `propose_intelligent_architecture` + `run_web_research` as last resort.
...
- `run_training_trial` — training (only optimization tool available in this campaign)
...
**CURRENT FOCUS: {selected_model} on {raw_data_source}**
**MANIFEST RULE: Before any `run_training_trial`, the manifest must exist. If switching to a new architecture, the FIRST objective MUST call `create_dataset_manifest` with ALL required params (n_fft, hop_length, gaf_image_size, video_num_segments, video_gaf_image_size). Never assume a manifest exists for a new architecture.**
...
ESCALATION GUIDANCE:
- Fresh start / < 10 trials → run multiple `run_training_trial` with varied hyperparameters
- Plateau (10-20 steps) → diversify `run_training_trial` configs + `generate_creative_hypotheses`
- Deep plateau (20+ steps) → `propose_intelligent_architecture` or `run_web_research`
...
- [ ] O1: Call `set_optimization_model(model_name="<arch_or_custom_file>")` to switch focus, then `create_dataset_manifest` with all required params (n_fft, gaf_image_size, hop_length, video_num_segments), then run initial `run_training_trial` with `representation_type="<rep>"` (and `custom_architecture_file="<file>"` if using a custom arch).
...
- [ ] O2: If O1 succeeds, run multiple `run_training_trial` calls with varied hyperparameters to begin optimization of the new architecture.
...
- `run_training_trial` (additional parallel calls) = 1 step each
...
10. **VARY HYPERPARAMETERS**: When trials plateau, switch to orthogonal dimensions (e.g., if `learning_rate`/`dropout_rate` are exhausted → vary `num_conv_layers`, `filters`, `kernel_size`, `dense_units`, `weight_decay` instead).
...
12. **MANIFEST PREREQUISITE**: Before calling `run_training_trial` for ANY architecture, verify the manifest exists by checking `list_available_datasets()` ONCE per cycle. If the manifest is missing, call `create_dataset_manifest` with explicit params (n_fft, hop_length, gaf_image_size, video_num_segments, video_gaf_image_size) BEFORE any training or sweep call. Never rely on silent auto-creation. **CRITICAL: `list_available_datasets` is BLOCKED after the first call — a code-level guard will silently drop any duplicate call and waste your step. Once you have seen its output, it will not run again. Proceed immediately to training.**

   **HOW `create_dataset_manifest` WORKS — read this before calling it:**

   A manifest is a named configuration record stored in metadata. It does two things:
   (a) stores the preprocessing params (n_fft, hop_length, gaf_image_size, etc.) so data generation is reproducible;
   (b) tracks which `representation_type` data files (`.npy`) have been generated for that manifest.

   **Lifecycle — what actually happens at each step:**
   - `create_dataset_manifest(manifest_name, params)` → registers the manifest in metadata. Does NOT generate data yet. Returns immediately.
   - `run_training_trial(manifest_name, representation_type, ...)` → checks if the `.npy` file for that `representation_type` exists under the manifest. If missing, it auto-generates it before training starts. You do NOT need to call `generate_representation` manually.
   - The manifest's `generated_representations` dict is updated each time a new representation is generated. `list_available_datasets()` shows which ones exist.

   **The `representation_type` you pass to `run_training_trial` controls which data file is loaded — it does NOT have to match the manifest's default `representation_type`.** A single manifest (e.g. `data_paper_mine_csv`) can hold `2D_GAF`, `3D_DYNAMIC_GAF`, `2D_SPECTROGRAM` etc. Each is a separate `.npy` file. Training on `3D_DYNAMIC_GAF` loads `3D_DYNAMIC_GAF_data.npy` from the manifest folder, regardless of what `representation_type` is stored in the manifest metadata.

   **When to call `create_dataset_manifest` vs when NOT to:**
   - CALL it when the manifest name does not appear in `list_available_datasets()` output.
   - DO NOT call it if the manifest already exists — even if you want a different `representation_type`. Just call `run_training_trial` with the new `representation_type` directly; data will be auto-generated.
   - DO NOT call it to "re-configure" an existing manifest. Params are fixed at creation time. If you need different preprocessing params (e.g. different n_fft), create a NEW manifest with a NEW name.

   **Mandatory params for timeseries manifests:**
   ```json
   {{"tool_name": "create_dataset_manifest", "args": {{
     "manifest_name": "my_manifest_v1",
     "params": {{
       "representation_type": "3D_DYNAMIC_GAF",
       "n_fft": 512,
       "hop_length": 64,
       "gaf_image_size": 48,
       "video_num_segments": 8,
   ...
   ```
   Always pass `representation_type` explicitly in params — the system will reject manifests where it cannot determine the type.
...
13. **TRIAL DIVERSITY**: Every batch of parallel `run_training_trial` calls MUST cover AT LEAST 5 distinct hyperparameter dimensions. Cover at minimum: `learning_rate`, `dropout_rate`, `filters`, `num_conv_layers`, and `dense_units`. Add `kernel_size`, `weight_decay`, or `batch_size` for further diversity.
...
1. **ANALYZE & TUNE** — Use analytical tools. Run data-driven targeted `run_training_trial` calls.
...
2. **INNOVATE** — If tuning fails after 20+ steps stuck, call the Innovation Team.
   ...
   - **POST-INNOVATION MANDATORY (immediately after `propose_intelligent_architecture` succeeds):**
     1. Run a test training trial with ALL THREE required arguments:
        ...
        ```
     2. If test trial succeeds → run multiple parallel `run_training_trial` calls with the same `custom_architecture_file`, `manifest_name`, and `representation_type`, varying learning_rate, dropout_rate, filters, dense_units, weight_decay across calls.
...
4. **ABANDON CRITERIA** (ALL 3 must be true — not just one):
   - ✓ 20+ total trials completed
   - ✓ Multiple varied `run_training_trial` batches failed to improve
   - ✓ Architecture comparison shows fundamental limitations
...
- `plateauing_try_harder`: 10-20 steps stuck → run 3-5 parallel `run_training_trial` with orthogonal hyperparameter configs
- `plateauing_innovate`: 20+ steps stuck → Innovation Team mandatory
- `in_decline` → try `propose_intelligent_architecture` before abandoning
...
`create_dataset_manifest` (only when manifest is absent from `list_available_datasets`):
```json
{{"tool_name": "create_dataset_manifest", "args": {{
  "manifest_name": "my_manifest_v1",
  "params": {{
...
```
**Do NOT call this if the manifest already exists. Pass `representation_type` in params — omitting it causes a hard error.**
...
**Pattern B — parallel training + CPU research:**
```json
[
  {{"tool_name": "run_training_trial", "args": {{
    "representation_type": "1D_CNN", "manifest_name": "data_paper_mine_csv",
    "num_conv_layers": 6, "filters": 128, "dense_units": 256,
    "learning_rate": 0.0005, "weight_decay": 1e-4, "batch_norm": 1, "epochs": 50, "patience": 15
  }}}},
  {{"tool_name": "query_research_archive", "args": {{"query": "deep CNN timeseries overfitting regularization", "top_k": 5}}}}
]
```

**Pattern C — plateau: immediately switch to orthogonal params + memorize what you learned:**
```json
[
  {{"tool_name": "run_training_trial", "args": {{
    "representation_type": "1D_CNN", "manifest_name": "data_paper_mine_csv",
    "num_conv_layers": 8, "filters": 256, "kernel_size": 5,
    "dense_units": 512, "dropout_rate": 0.3, "learning_rate": 0.0001,
    "weight_decay": 5e-4, "batch_norm": 1, "epochs": 50, "patience": 15
  }}}},
  {{"tool_name": "run_training_trial", "args": {{
    "representation_type": "1D_CNN", "manifest_name": "data_paper_mine_csv",
    "num_conv_layers": 4, "filters": 64, "kernel_size": 3,
    "dense_units": 128, "dropout_rate": 0.5, "learning_rate": 0.001,
    "weight_decay": 1e-5, "batch_norm": 1, "epochs": 50, "patience": 15
    }}
  }}}},
  {{"tool_name": "memorize_finding", "args": {{
...
- `best_trials_stats` / `worst_trials_stats` = **averages** over the top/bottom 25% of ALL historical trials. The `mean_accuracy` here is a historical aggregate — it may look lower than recent trial results because it includes old experiments.
...
- The `single_best_trial` is always more authoritative than `best_trials_stats.mean_accuracy` for recent results.
...
- Did the Decider violate any constraints? (e.g. omitted custom_architecture_file, called unavailable tools)
