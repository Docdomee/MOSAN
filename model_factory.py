...
def build_2d_image_cnn(input_shape, num_classes, params):
    """
    Dedicated builder for Standard Images (e.g. CIFAR-10, PNGs).
    Includes Normalization (0-255 -> 0-1).
    """
    min_dim = min(input_shape[0], input_shape[1])
    max_layers = int(np.log2(min_dim)) if min_dim > 0 else 1
    requested_layers = params.get("num_conv_layers", 2)
    ...
    if num_conv_layers < requested_layers:
        print(
            f"[Builder Warning] 2D_IMAGE: Requested {requested_layers} layers, but data dimension only supports {num_conv_layers}. Adjusting automatically."
        )

    inputs = keras.Input(shape=input_shape)
    x = inputs
    
    # --- Normalization (Specific to Standard Images) ---
    # Rescale [0, 255] -> [0, 1]
...
MODEL_BUILDERS = {
    ...
    # IMAGENET COMPETITION MODULE
    # Loaded lazily — only imported when this key is referenced.
    # Architecture: ResNet-50/101/152 + dual cls+bbox head.
    # Registered with a factory function to defer the import.
    "IMAGENET_DETECTOR": None,  # Dynamically resolved — see get_imagenet_builder()
}


def get_imagenet_builder():
    """
    Lazy loader for the ImageNet detector architecture.
    Import is deferred to avoid loading the heavy architecture module
    during normal optimization/report runs.

    Returns:
        build_model function from resnet_152_imagenet.py
    """
    from cluster.library.architectures.resnet_152_imagenet import build_model
    return build_model